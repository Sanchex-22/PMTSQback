...
async function main() {
  ...
  // Delete all existing records to avoid duplicates on re-run
  await prisma.course.deleteMany({});
  console.log(`Deleted existing Course records.`);
  ...
  for (const courseData of courses) {
    const course = await prisma.course.create({
      data: {
        ...
        price_panamanian: courseData.price_panamanian ?? 0,
        price_panamanian_renewal: courseData.price_panamanian_renewal ?? 0,
        price_foreign: courseData.price_foreign ?? 0,
        price_foreign_renewal: courseData.price_foreign_renewal ?? 0,
      },
    });
    console.log(`Created course with id: ${course.id} - ${course.name}`);
  }
